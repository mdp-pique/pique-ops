...
## Automatic Requests tickets — as of 2026-09-24

Migration `20260924230000_auto_request_tickets.sql`, exception-safe AFTER triggers (never block the source write): `reservations` → `direct_booking_id_check` (accepted, upcoming, `booking_source = 'direct'`, ref `direct_id:{reservation}`) and `pet_fee` (accepted, upcoming, `raw_hospitable_data->'guests'->>'pet_count' > 0`, ref `pet_fee:{reservation}`); `packnplay_requests.requested = true` → `pack_n_play` (ref `packnplay:{reservation}`). Cancelled bookings / withdrawn pack-n-play requests resolve the open ticket. Tickets are created once (`on conflict do nothing`) with their checklist; due dates come from the clock. Helpers `upsert_request_ticket` / `resolve_request_ticket` are revoked from public/anon/authenticated.

## Team assignment — as of 2026-09-25

Migration `20260925000000_ticket_teams.sql`: `teams`, `team_members`, `tickets.assignee_team_id`, and `ticket_type_clocks.default_team_id`. A ticket can sit with a team, a person, or both (team owns it, a teammate picks it up; assigning a team clears the person). BEFORE INSERT trigger `tickets_default_team` fills the type's default team when nobody is assigned (works for automation-created tickets too). "Mine" = assigned to me, or to one of my teams with no person yet; "Unassigned" = no person and no team. Managed at `/admin/teams` by `admin` / `ops_manager` (RLS enforces the same).

## Calendar — as of 2026-09-24

PRD §7.9. `/calendar` (`src/lib/data/calendar.ts`, `CalendarView.tsx`) is a read-only week view with toggleable layers: reservations, cleans, maintenance, customer service, reviews. Open tickets sit on their due day, closed ones on their closed day, and past-due open tickets carry into an Overdue row. Cleans come from `cleaning_shift_check`, which is service-role only (no authenticated policy - deliberately not changed). The page reads it server-side with the admin client after checking the user has a profile, selecting only display fields. Future days come from `connecteam_shifts`, a read-only hourly copy of the Connecteam schedule (n8n `Pique-Connecteam-Shifts-Sync`, id `rfFercNzL7BUJQfM`; notes are stripped because they hold door codes). `cleaning_schedule_reconcile` / `cleaning_schedule_orphans` compare upcoming checkouts to those shifts by the reservation code in the shift title - groundwork for replacing the Zapier Zap that creates shifts today (the Zap is live and stays untouched until sign-off). QC and recurring tasks are the next phases.

The replacement (migration `20260925030000`, n8n `Pique-Cleaning-Shifts-From-Bookings` id `TCsHEpxNcHxqFOKd`) drives shifts from bookings: one linked shift per checkout (`cleaning_shift_links`), every decision logged in `cleaning_shift_actions`. It is gated by `automation_flags.cleaning_shifts_mode`: `dry_run` only logs and adopts existing shifts, `live` writes to Connecteam. **Live since 2026-09-25 16:18 MT; the Zapier Zap is off.** Setting the mode back to `dry_run` is the emergency stop. Never turn the Zap back on while this is live, or both will create shifts. `cleaning_shifts_live_actions` / `cleaning_shifts_batch` limit what a live run may send (use them to stage anything risky). Never store Connecteam response bodies or shift notes: they carry door codes. See PRD §7.9.
...
- **Draft it anyway** (2026-09-25): the AI rubric (copied from the n8n monitor) treats factors outside the host's control as not removable, so some reviews the team wants to appeal always came back "No violation found", even after regenerating with feedback. When the verdict is no-violation, staff can pick the grounds and press **Draft it anyway**. The model then writes the strongest honest request for those grounds and adds its read on the odds. It still must not invent facts. Logging a no-violation result now asks for confirmation first, because it closes the removal case.
