...
- **Parking form (`vehicle_registration`) - source answered 2026-09-24:** guest submissions come from a **GHL form** and land in a **Slack channel** (not via any n8n workflow). Plan: a new, additive n8n workflow takes the GHL form submission (form webhook, or failing that the Slack post), matches it to the reservation (confirmation code if the form asks for it, else guest name + property + dates), and upserts a `vehicle_registration` ticket on `external_ref = 'parking:{reservation_id}'` with the plate filled in and "Plate received" ticked. Until then, tickets are created by hand. Still open: the form's exact fields (does it ask for the confirmation code?), which properties besides Fire Mountain Lodge #213 need registration, and how the building is notified (portal, email, phone).
