...
- **Phase 2a, upcoming cleans (live 2026-09-25):** n8n `Pique-Connecteam-Shifts-Sync` (`rfFercNzL7BUJQfM`, hourly, read-only) copies the Connecteam Job Scheduler's shifts from 7 days back to ~6 months ahead into `connecteam_shifts` (notes stripped - they hold door codes). The calendar shows future cleans from it; today and earlier still come from `cleaning_shift_check`. QC gets its own layer once we know how QC is scheduled in Connecteam.
- **Replacing the Zapier Zap (in progress):** the Zap (Hospitable `reservation.changed` → sheet "Connecteam Lookup" by exact property name → Connecteam find-shift → create shift if none) only ever creates shifts, so date changes and cancellations leave shifts behind, two quick changes create duplicates, and 17 of the sheet's 78 names no longer match Hospitable so those properties never get a shift. Views `cleaning_schedule_reconcile` (every upcoming Edmonton/Calgary checkout vs Connecteam, matched on the reservation code in the shift title) and `cleaning_schedule_orphans` (shifts with no live booking) measure it. First run (2026-09-25, 209 checkouts): 151 ok, 44 with duplicate copies (79 extra shifts), 14 missing (10 are Hicham - Elbow Dr in Calgary, not in the sheet); 57 orphan shifts (19 dates moved, 20 cancelled/not accepted, 11 hand-made without a code, 7 unknown code).
- **Replacement built (2026-09-25, dry run):** migration `20260925030000_cleaning_shifts_from_bookings.sql` + n8n `Pique-Cleaning-Shifts-From-Bookings` (`TCsHEpxNcHxqFOKd`, every 10 min, not yet published). Bookings are the source of truth. Connecteam is only read so existing Zap shifts get adopted instead of duplicated. Each accepted Edmonton/Calgary checkout (today to +183 days, property has a row in `cleaning_property_jobs`) gets exactly one linked shift (`cleaning_shift_links`). `plan_cleaning_shift_actions()` decides: adopt the best existing shift with the booking's code, otherwise create; move a linked shift when the checkout date changes; recreate it if it was deleted in Connecteam; remove other empty unpublished copies (`remove_duplicate`); remove the shift of a cancelled booking if nobody claimed it and it's unpublished, otherwise post to Slack for a human (`flag_claimed`). Every decision is a row in `cleaning_shift_actions` (unique `dedupe_key`, mode `dry_run`/`live`, status planned/done/failed/skipped). Shifts match the Zap (title `<property> - <code>`, open shift, unpublished, admin approval, notes = guest, # guests, info and access instructions read live from the "Connecteam Lookup" sheet, never stored). One change: time is the property's checkout time to +4h, capped at check-in (10:00-14:00 for most). The Zap's shifts land at 17:00/19:00 because of a timezone bug and get hand-fixed today.
  - Switch: `automation_flags.cleaning_shifts_mode`. In `dry_run` the planner only logs and adopts, and nothing is sent to Connecteam. In `live` it returns up to 50 planned actions per run for n8n to execute, and n8n reports back through `complete_cleaning_shift_action()`.
  - First dry run: 190 existing shifts adopted; planned 4 creates, 20 removals of cancelled/unclaimed shifts, 95 duplicate removals; no moves.
  - **Cutover (needs sign-off):** review `cleaning_shift_actions`, turn the Zap off, publish the workflow, set the flag to `live`, and supervise the first run. The Connecteam write shapes (POST create, PUT move, DELETE) were inferred from the read API and must be confirmed on the first creates before the bulk removals run.
  - **Cleanup done (2026-09-25, supervised manual runs, Zap still on):** create, then delete, confirmed against Connecteam one at a time. Then 14 creates (4 renamed properties plus 10 Hicham - Elbow Dr checkouts the Zap never covered), 20 cancelled-booking shifts removed and 98 empty duplicate copies removed, 0 failures. Result: 204 of 204 upcoming checkouts have exactly one shift. Move (PUT) is still unverified because no moves were needed.
  - Safety added (migration `20260925040000`): flags `cleaning_shifts_live_actions` and `cleaning_shifts_batch` limit what a run may send, and a queued action the current plan no longer asks for is skipped instead of sent. Migration `20260925050000`: properties missing from `cleaning_property_jobs` fall back to `cleaning_job_map` when it names exactly one job, so properties set up in Connecteam later are picked up automatically. The action log stores only status and shift id, never the notes (they hold door codes).
  - **Cut over 2026-09-25 16:18 MT:** Zap "New Main - Connecteam Updated Booking" turned off, workflow published, mode `live`. The first scheduled run (16:20) succeeded, and 207 of 207 upcoming checkouts are ok. Watch the first real date change (move is still unverified).
