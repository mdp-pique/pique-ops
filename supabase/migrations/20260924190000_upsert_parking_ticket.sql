-- vehicle_registration tickets from the GHL parking form (PRD §9, §12).
-- Called by the n8n "Pique-Parking-Form-To-Ticket" workflow with the raw form
-- payload. Field names in GHL forms are free-form, so fields are found by key
-- pattern rather than fixed names. Idempotent: one ticket per reservation
-- (external_ref 'parking:{reservation_id}'); a resubmission updates it.
...
begin
  ...
  -- Flatten nested objects into lowercased key -> text value. Shallowest key wins,
  -- so the form's own fields beat GHL's nested location/workflow objects.
  with recursive walk(k, v, depth) as (
    select lower(e.key), e.value, 0 from jsonb_each(coalesce(p_payload, '{}'::jsonb)) e
    union all
    select lower(e.key), e.value, w.depth + 1
  ...
  select value into v_name from jsonb_each_text(v_fields)
    where key ~ '^(full_?name|name|guest_?name|contact_?name)$' limit 1;
  if v_name is null then
    select nullif(trim(coalesce(v_fields->>'first_name', '') || ' ' || coalesce(v_fields->>'last_name', '')), '') into v_name;
  end if;
  select value into v_email from jsonb_each_text(v_fields)
    where key ~ 'email' and value ~ '@' limit 1;
